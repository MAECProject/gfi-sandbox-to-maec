...
#
# Generated Fri Nov 30 13:16:45 2012 by generateDS.py version 2.7c.
#
...
class AddressObjectType(common_types_1_0.DefinedObjectType):
    ...
    def exportLiteralChildren(self, outfile, level, name_):
        ...
        if self.Address_Value is not None:
            showIndent(outfile, level)
            outfile.write('Address_Value=%s,\n' % quote_python(self.Address_Value).encode(ExternalEncoding))
        if self.Ext_Category is not None:
            showIndent(outfile, level)
            outfile.write('Ext_Category=%s,\n' % quote_python(self.Ext_Category).encode(ExternalEncoding))
        if self.VLAN_Name is not None:
            showIndent(outfile, level)
            outfile.write('VLAN_Name=%s,\n' % quote_python(self.VLAN_Name).encode(ExternalEncoding))
        if self.VLAN_Num is not None:
            showIndent(outfile, level)
            outfile.write('VLAN_Num=model_.common_types_1_0.IntegerObjectAttributeType(\n')
